...
export default function volunteerCheckinRoutes(volunteersCollection) {
  router.post("/", async (req, res) => {
    try {
      ...
      // CRITICAL FIX: Block the update if they are already checked in
      if (volunteer.checkedIn === true) {
        return res.json({ success: false, message: "This volunteer has already been checked in!" });
      }

      // Update volunteer check-in status safely
      await volunteersCollection.updateOne(
        { qrId },
        {
          $set: {
